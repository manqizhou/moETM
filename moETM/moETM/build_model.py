...
class decoder(nn.Module):
    def __init__(self, mod1_dim, mod2_dim, z_dim, emd_dim, num_batch):
        ...
        if self.num_batch>1:
            self.mod1_batch_bias = nn.Parameter(torch.randn(self.num_batch, mod1_dim))
            self.mod2_batch_bias = nn.Parameter(torch.randn(self.num_batch, mod2_dim))

    def forward(self, theta, batch_indices):
