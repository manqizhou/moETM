...
def plot_loss(epochs, train_losses, vald_losses, title, savepath):
    plt.figure()
    plt.plot(epochs, train_losses, color="tab:blue", label="Training Loss")
    plt.plot(epochs, vald_losses, color="tab:orange", label="Validation Loss")
    plt.xlabel("Epochs")
    plt.ylabel("BCE Loss")
    plt.title(title)
    plt.legend()
...
def calculate_micro_metrics(target, pred_probas):
    '''
        target: (N, cluster_count)
        pred_probas: (N, cluster_count)
    '''
    assert target.shape == pred_probas.shape
    target = target.flatten()
    pred_probas = pred_probas.flatten()
    # calculate micro AUROC
    auroc = roc_auc_score(target, pred_probas)
    # calculate micro AUPR
    prec, rec, thr = precision_recall_curve(target, pred_probas)
    auprc = auc(rec, prec)
    return auroc, auprc

def calculate_macro_metrics(target, pred_probas):
    '''
        target: (N, cluster_count)
        pred_probas: (N, cluster_count)
    '''
    ...
    for idx in range(target.shape[1]):
        cur_targets = target[:,idx]
        cur_probs = pred_probas[:,idx]
        temp.append(roc_auc_score(cur_targets, cur_probs))
    auroc = np.mean(temp)
    # calculate macro AUPR
    temp = []
    for idx in range(target.shape[1]):
        cur_targets = target[:,idx]
        cur_probs = pred_probas[:,idx]
        prec, rec, thr = precision_recall_curve(cur_targets, cur_probs)
        temp.append(auc(rec, prec))
    auprc = np.mean(temp)
    return auroc, auprc

def plot_multilabel_auroc(epochs, train_micro_aurocs, train_macro_aurocs, vald_micro_aurocs, vald_macro_aurocs, title, savepath):
    plt.figure()
    plt.plot(epochs, train_micro_aurocs, color="tab:blue", label="Training (Micro)")
    plt.plot(epochs, train_macro_aurocs, color="tab:cyan", label="Training (Macro)")
    plt.plot(epochs, vald_micro_aurocs, color="tab:orange", label="Validation (Micro)")
    plt.plot(epochs, vald_macro_aurocs, color="maroon", label="Validation (Macro)")
    plt.xlabel("Epochs")
    plt.ylabel("AUROC")
    plt.title(title)
    plt.legend()
...
def plot_multilabel_aupr(epochs, train_micro_auprs, train_macro_auprs, vald_micro_auprs, vald_macro_auprs, title, savepath):
    plt.figure()
    plt.plot(epochs, train_micro_auprs, color="tab:blue", label="Training (Micro)")
    plt.plot(epochs, train_macro_auprs, color="tab:cyan", label="Training (Macro)")
    plt.plot(epochs, vald_micro_auprs, color="tab:orange", label="Validation (Micro)")
    plt.plot(epochs, vald_macro_auprs, color="maroon", label="Validation (Macro)")
    plt.xlabel("Epochs")
    plt.ylabel("AUPR")
    plt.title(title)
    plt.legend()
...
def plot_multilabel_macro_auroc(epochs, train_macro_aurocs, vald_macro_aurocs, title, savepath):
    plt.figure()
    plt.plot(epochs, train_macro_aurocs, color="tab:cyan", label="Training (Macro)")
    plt.plot(epochs, vald_macro_aurocs, color="maroon", label="Validation (Macro)")
    plt.xlabel("Epochs")
...
def plot_multilabel_macro_aupr(epochs, train_macro_auprs, vald_macro_auprs, title, savepath):
    plt.figure()
    plt.plot(epochs, train_macro_auprs, color="tab:cyan", label="Training (Macro)")
    plt.plot(epochs, vald_macro_auprs, color="maroon", label="Validation (Macro)")
    plt.xlabel("Epochs")
